...
-- IF (조건식) THEN (실행문1) 
-- ESLIF (조건식2) THEN (실행문2) -- 계속 늘릴 수 있음
-- ELSE (실행문3) 
-- END IF;
...
-- LOOP를 실행할 떄 조건이 처음부터 FALSE이면 한 번도 수행되지 않을 수 있음
...
-- PL/SQL 예외처리
-- 시스템 오류(메모리 초과, 인덱스 중복키 등)는 오라클이 정의하는 에러로
-- 보통 PL/SQL 실행 엔진이 오류 조건를 탐지하여 발생시키는 예외임.
