...
-- 트리거 생성 후 지정된 테이블에 명령어를 실행하면 트리거가 작동됨.
-- 신입사원이 입사하였습니다.
-- 1 행 이(가) 삽입되었습니다.
...
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- EMPLOYEE 테이블에 무조건 UPDATE가 되면 무조건 실행하는 것을 방지
    THEN
...
INSERT INTO PRODUCT_IO VALUES(SEQ_PRODUCTIO_IOCODE.NEXTVAL, 10, SYSDATE, 10, '입고');
INSERT INTO PRODUCT_IO VALUES(SEQ_PRODUCTIO_IOCODE.NEXTVAL, 11, SYSDATE, 5, '출고');
COMMIT;
