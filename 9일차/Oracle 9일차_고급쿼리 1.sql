...
-- ROWNUM : 각 행에 대한 일련번호, 오라클레서 내부적으로 부여하는 컬럼
...
-- 2. WITH
-- 서브쿼리에 이름을 붙여주고 인라인뷰로 사용시 서브쿼리의 이름을 FORM절에 기술할
-- 수 있음 같은 서브쿼리가 여러번 사용될 경우 중복 작성을 피할 수 있고 실행속도도
-- 빨라지는 장접이 있음
-- 사용방법
-- WITH 서브쿼리명 AS (서브쿼리)
-- SELECT * FROM (서브쿼리명)
-- 예제) 급여 TOP5인 직원의 전체정보를 출력하시오
WITH TOP5_SAL AS 
(SELECT * 
FROM EMPLOYEE 
...
-- 4. LEVEL 의사컬럼(PSEUDO COLUMN), 계층 정보를 나타내는 가상 컬럼이며
...
INSERT INTO MENU_TBL
VALUES(1001, '서브메뉴A1', 1000);
INSERT INTO MENU_TBL
VALUES(1002, '서브메뉴A2', 1000);
INSERT INTO MENU_TBL
VALUES(1003, '서브메뉴A3', 1000);
...
-- 특정 컬럼 기준으로 랭킹을 부여함, 중복 순위 다음은 해당 개수만큼 건너뛰고 반환함
-- 예제) 회사의 연봉 순위를 출력하시오
...
-- 4.2 DENSE_RANK() OVER
-- -> 중복 순위 상관없이 순차적으로 반환, 빠짐 없이 빽빽한 순위를 부여함
...
--DELETE FROM EMPLOYEE WHERE EMP_NAME IN ('일용자', '이용자', '이채민');
--COMMIT;
