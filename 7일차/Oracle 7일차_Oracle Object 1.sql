...
-- - 사용자에게 권한을 부여할 때 한 개씩 부여하게 된다면 권한 부여 및 회수의 관리가 
-- 불편하므로 사용하는 객체임
SELECT * FROM DBA_SYS_PRIVS
ORDER BY 2 ASC; -- SYSTEM 계정 실행(RED)

-- *DCL(Data Control Laguage) : GRANT / REVOKE
-- 권한부여 및 회수는 관리자 세션(빨간색)에서 사용 가능
-- 관리자 계정
-- 1. SYSTEM : 일반 관리자, 대부분의 권한을 가지고 있음
-- 2. SYS : DB 생성/삭제 권한이 있음. 로그인 옵션으로 AS SYSDB를 적어줘야 함
GRANT CONNECT, RESOURCE TO KH;
-- CONNECT, RESOURCE는 권한이 모여있는 ROLE이다
-- ROLE에 부여된 시스템 권한 확인

SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'CONNECT'; -- 관리자 계정에서 사용
SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'RESOURCE'; -- 관리자 계정에서 사용
-- CREATE VIEW 권한이 RESOURCE ROLE에 없었기 떄문에 따로 권한부여를 해줌
...
-- - KEY와 VALUE 형태로 생성ㅇ이 되며 KEY에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된
-- 주소값이 저장됨
-- - 장점 : 검색속도가 빨라지고 시스템에 걸리는 부하를 줄여서 시스템 전체 성능을 향상시킬 수 있음
-- - 단점 : 인덱스를 위한 추가 저장 공간이 필요하고 인덱스를 생성하는데 시간이 걸림
-- 데이터의 변경 작업이 자주 일어나는 테이블에 INDEX 생성시 오히려 성능저하가 발생할 수 있음
...
WHERE EMP_ID LIKE '2%';
SET TIMING OFF;
-- F10으로 실행하여 오라클 PLAN 확인가능, 튜닝 시 사용됨
