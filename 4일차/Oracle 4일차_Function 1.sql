...
SELECT KENGTH(EMAIL) FROM EMPLOYEE;
SELECT FLOOR(SYSDATE - HIRE_DATE) FROM EMPLOYEE;
SELECT SUM(SALARY) FROM EMPLOYEE;
-- 1.1 단일행 함수의 종류
-- 1.1.1 숫자 처리 함수
--      - ABS, MOD, TRUC, FLOOR, ROUND, CEIL
-- 1.1.2 문자 처리 함수
--      a. LENGTH, LENGTHB : 길이 구함
--      b. INSTR, INSTRB   : 위치 구함
--      c. LPAD, RPAD      : 빈 곳을 채워줌
--      d. STRIM/RTRIM     : 특정문자 제거(공백제거)
--      e. TRIM
--      f. SUBSTR          : 문자열 잘라줌
--      g. CONCAT          : 문자열 합쳐줌        CONCAT은 최대 2개, 그 이상은 ||(이건 제한 없음)
--      h. REPLACE         : 문자열 바꿔줌
--      함수명(값1, 값2, ...)으로 적고 값에는 리터럴이나 컬럼명을 넣어주면 됨
-- 1.1.3 날짜 처리 함수
...
SELECT EMP_NAME "직원명", DEPT_CODE "부서 코드"
...
-- EMPLOYEE 테이블에서 입사일이 00/01/01 ~ 10/01/01 사이인 직원의 정보를 출력하시오.
SELECT * FROM EMPLOYEE
...
SELECT HIRE_DATE, EXTRACT(YEAR FROM HIRE_DATE)||'년 '
...
-- DCODE 함수
-- 사용법 : DCODE(변하는 값, 조건값, 참일때 값, 조건값, 참일때 값, 모두 아닐때 값)
-- 예시 : DCODE(변하는 값, 1, '빨강', 2, '노랑', '파랑')
-- CASE 함수
-- 사용법 : CASE WHEN 변하는 값 = 조건값 THEN 참일때 값 WNEN 변하는 값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
...
-- 성별 별 급여의 평균(정수처리), 급여의 합계, 인원수를 조회한 뒤 인원수로 내림차순을 정렬하시오.
...
-- 그룹 함수 결과이지만 GROUP BY를 쓰면 기분 별로 결과가 나옴
...
-- SELECT문에서 조건을 걸 떄는 WHERE을 쓰지만
SELECT * FROM EMPLOYEE
WHERE SALARY >= 5000000;
-- GROUP BY한 결과값에 조건을 걸 때에는 HAVING을 쓴다.

-- 부서별 인원 수가 5명 미만/초과인 레코드를 출력하세요.      
...
-- 실습문제2
-- 부서내 직급별 인원수가 3명 이상인 직원의 부서코드, 직급코드, 인원수를 출력하세요.
...
-- 실습문제3
-- 매니저가 관리하는 사원이 2명 이상인 매니저 아이디와 사원수를 출력하세요.
