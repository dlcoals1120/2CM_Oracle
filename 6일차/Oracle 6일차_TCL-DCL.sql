...
-- ORCLE DBMS 트랜잭션?
-- INSERT 수행 OR DELETE 수행이 되었다면 그 뒤에 추가 작업이 
-- 있을 것으로 간주하고 처림 -> 트랜잭션이 걸렸다.
DESC USER_GRADE;
...
-- 2. ROLLBACK : 트랜잭션이 작업을 모두 취소하고 가장 최근 COMMIT 시점으로 이동.
-- 3. SAVEPOINT <savepoint명> : 현재 트랜잭션 작업 시점에 이름을 저장함.
