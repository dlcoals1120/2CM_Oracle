...
/*
    1.1 개요
    - 실제 테이블에 근거한 논리적인 가상의 테이블(사용자에게 하나의 테이블처럼
    사용 가능하게 함)
...
-- 1.2.1 Stored View(저장O) - 이름을 붙여서 저장함
    ...
    FROM EMPLOYEE;
-- 주의 : VIEW를 만들 때는 권한이 필요함, 권한명 : CREATE VIEW
-- SYSTEM 계정으로 권한 부여
...
-- 생성하여라.
CREATE VIEW V_EMP_INFO (사원아이디, 사원명, 직급명, 부서명, 관리자명, 입사일)
AS 
SELECT EMP_ID, EMP_NAME, JOB_NAME, NVL(DEPT_TITLE, '미정')
...
SELECT SEQ_KH_USER_NO.NEXTVAL FROM DUAL; -- 첫 NEXTVAL은 시퀀스를 게시함. 필수조건
...
COMMENT ON COLUMN ORDER_TBL.PRODUCT_CNT IS '주문개수';
COMMENT ON COLUMN ORDER_TBL.ORDER_DATE IS '주문일짜';
...
VALUES(SEQ_MEMBER_ID.NEXTVAL, '홍길동', 20, SEQ_MEMBER_JOIN.COM.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_MEMBER_ID.NEXTVAL, '고길동', 30, SEQ_MEMBER_JOIN.COM.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_MEMBER_ID.NEXTVAL, '외길동', 40, SEQ_MEMBER_JOIN.COM.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_MEMBER_ID.NEXTVAL, '고길동', 50, SEQ_MEMBER_JOIN.COM.NEXTVAL);
