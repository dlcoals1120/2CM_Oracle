...
-- WHERE EMP_ID = '200';
ORDER BY EMP_ID ASC; -- DESC : 내림차순, ASC : 오름차순
...
, SALARY*12+SALARY*NVL(BONUS,0) "총 수령액"
...
SELECT EMP_NAME, SALARY, NVL(BONUS, 0 ) -- NVL : NULL을 0으로 바꿔줌
...
FROM EMPLOYEE
-- WHERE PHONE NOT LIKE '010________' : _ 사용할 경우 자리수 고정됨
...
, HIRE_DATE "고용일", ROUND(ENT_DATE - HIRE_DATE) "근무 기간", ENT_DATE "퇴직일"
FROM EMPLOYEE
WHERE ENT_YN = 'Y';
-- 문제3.
...
-- ORDER BY 근속년수 ASC;
ORDER BY 3 ASC; -- 위치 번호로 가능 ex) 1 = 이름, 2 = 급여, 3 = 근속년수
-- 문제4.
...
SELECT EMP_NAME "이름", EMP_NO "주민번호", SALARY "급여"
, NVL(DEPT_CODE, '없음') "부서코드"
FROM EMPLOYEE
WHERE SALARY BETWEEN 2000000 AND 3000000
AND EMP_NO LIKE '__04__-2%'
ORDER BY EMP_NO DESC; -- ORDER BY는 무조건 마지막 문장!!
...
FROM EMPLOYEE
WHERE EMP_NO LIKE '______-1%' -- '%-1%'도 가능
AND BONUS IS NULL
ORDER BY 1 ASC;
...
-- 실습2
-- 부서배치를 받지 않았지만 보너스를 지급하는 직원 전체 정보 조
SELECT * FROM EMPLOYEE
