...
-- DROP TABLE, ...
-- ALTER로 할 수 있었던 것
-- 컬럼 추가, 컬럼 삭제, 컬럼 수정(데이터 타입, 이름,), 테이블명 수정 2가지
...
-- ex) USER_GRADE가 가지고 있는 제약조건 조회하여 PK 삭제 후 같은 이름으로 재생성
-- 제약조건 수정은 삭제하고 추가하는 것, 단 이름은 바꿀 수 있음
ALTER TABLE USER_GRADE
RENAME CONSTRAINT PK_GRADE_CODE TO PRIMARY_GR_CDOE;
-- 조회
...
-- ex04) USER_FOREIGN_KEY 테이블에 REG_DATE DATE 컬럼의 이름을 USER_DATE로 변경해주세요
ALTER TABLE USER_FOREIGN_KEY
...
DESC USER_FOREIGN_KEY;
-- USER DATE        VARCHAR2(10) -> USER_DATE       DATE

-- ex05) USER_FOREIGN_KEY 테이블에 이름을 USER_TBL로 바꿔주세요 (2가지)
ALTER TABLE USER_FOREIGN_KEY
RENAME TO USER_TBL;
...
SELECT * FROM USER_CONSTRAINTS WHERE TALBE_NAME = 'USER_TBL';
